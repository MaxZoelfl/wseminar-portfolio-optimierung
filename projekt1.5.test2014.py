...
# Turnover-Constraint fuer den RF-Optimierer (v4).
# Begrenzt die maximale einseitige Gewichtsaenderung je Rebalancing-Schritt.
# 0.30 = max. 30% des Portfolios wird pro Monat umgeschichtet.
# Begruendung: Gaerleanu & Pedersen (2013) zeigen, dass Turnover-Limits die
# realisierte Netto-Performance von ML-Strategien deutlich verbessern.
# Gilt NUR fuer RF (nicht MVO, der ohnehin geringen Turnover hat).
RF_TURNOVER_LIMIT = 0.30       # Max. 30% einseitiger Turnover je Monat
...
class MarkowitzLedoitWolf:
    ...
    def max_sharpe(self, mu: np.ndarray, cov: np.ndarray,
        ...
        v4-Erweiterung (optional): Turnover-Constraint
          Wenn w_prev und turnover_limit angegeben, wird die Summe der
          absoluten Gewichtsaenderungen begrenzt:
            sum(|w - w_prev|) / 2 <= turnover_limit
          Begruendung: Gaerleanu & Pedersen (2013) zeigen, dass explizite
          Turnover-Limits die Netto-Performance von Handelsstrategien
          signifikant verbessern, da Transaktionskosten superlinear mit
          dem Turnover wachsen.
        """
        n  = len(mu)
        ...
        # v4: Turnover-Nebenbedingung (nur wenn w_prev bekannt)
        if w_prev is not None and turnover_limit is not None:
            constraints.append({
                "type": "ineq",
                # sum(|w - w_prev|)/2 <= limit  <=>  limit - sum(...)/2 >= 0
...
class RFPortfolioOptimizer:
    ...
    def optimize(self,
        ...
        v4: Turnover-Constraint wird uebergeben wenn w_prev bekannt.
        """
        return self._mvo.max_sharpe(
            mu_predicted, cov,
            w_prev        = w_prev,
            turnover_limit = RF_TURNOVER_LIMIT,
        )
...
def run_backtest(asset_prices: pd.DataFrame,
    ...
    frontier_snapshots                  = []   # v3b: Frontier je Rebalancing

    # v4: Vormonat-Gewichte fuer Turnover-Constraint verfolgen
    w_prev_rf  = np.ones(n) / n   # Start: Equal Weight (aus Cash investiert)
    ...
    for i, month_end in enumerate(backtest_months[:-1]):
        ...
        if len(feat_rows) < n // 2:
            ...
            try:
                ...
                # v4: Turnover-Constraint — uebergebe Vormonat-Gewichte
        ...
        # ---- Frontier-Snapshot fuer Evolution-Plot (v3b) ------------------
        # Alle N Punkte der Effizienzlinie fuer diesen Rebalancing-Termin
        # speichern. Nur jeden 3. Monat um Laufzeit zu sparen (~40 statt 120).
        if i % 3 == 0:
            try:
                snap = mvo.efficient_frontier(mu_hist, cov_ann, n_points=60)
                if not snap.empty:
                    snap["date"]    = month_end
                    snap["w_tang"]  = [w_mvo] * len(snap)   # Tangentialpunkt
        ...
        # v4: Vormonat-Gewichte aktualisieren fuer naechsten Schritt
        w_prev_rf = w_rf.copy()
    ...
    return {
        ...
        "w_mvo_last"          : w_mvo,
        "w_rf_last"           : w_rf,
        "frontier_snapshots"  : frontier_snapshots,   # v3b
    }
...
def plot_frontier_evolution(frontier_snapshots: list,
                             output_path: str) -> None:
    """
    Abbildung 8 (v3b): Evolution der Effizienzlinie ueber den Backtestzeitraum.

    Jede Kurve entspricht einer Effizienzlinie zu einem Rebalancing-Termin.
    Farbe: Viridis-Colormap von dunkel (frueh) nach hell-gelb (spaet).
    Tangentialpunkte werden als kleine Rauten eingezeichnet.

    Wissenschaftliche Aussage: Die Frontier ist kein statisches Objekt,
    sondern verschiebt sich mit den sich aendernden Rendite- und
    Kovarianzschaetzungen — ein zentrales, oft uebersehenes Merkmal der MVO.
    """
    if not frontier_snapshots:
        log.warning("Keine Frontier-Snapshots vorhanden, ueberspringe Plot.")
        return
    ...
    cmap      = plt.cm.viridis
    norm      = plt.Normalize(vmin=0, vmax=n_dates - 1)
    ...
    for idx, date in enumerate(dates):
        ...
        color = cmap(norm(idx))
        alpha = 0.25 + 0.65 * (idx / max(n_dates - 1, 1))  # frueher = transparenter
        ...
        # Tangentialpunkt (Sharpe-Maximum je Frontier)
    ...
    cbar.set_label("Rebalancing-Zeitpunkt (frueh → spaet)", fontsize=9)

    # Tick-Labels der Colorbar auf Jahreszahlen setzen
    tick_indices = np.linspace(0, n_dates - 1, min(6, n_dates), dtype=int)
    cbar.set_ticks(tick_indices)
    cbar.set_ticklabels([str(dates[i].year) for i in tick_indices])

    # Erste und letzte Frontier hervorheben
    for idx, label, lw in [(0, f"Frontier {dates[0].strftime('%b %Y')} (Start)", 2.2),
    ...
    ax.set_title(
        "Evolution der Effizienzlinie ueber den Backtestzeitraum (2015–2024)\n"
        "Jede Kurve = Efficient Frontier zu einem Rebalancing-Termin | "
        "Rauten = Tangentialpunkte (max. Sharpe)",
        pad=12,
    )
    ax.set_xlabel("Annualisierte Volatilitaet (%)")
    ax.set_ylabel("Annualisierte Erwartungsrendite (%)")
    ...
    # Annotation: Erklarung fuer die Arbeit
    ax.annotate(
...
def main():
    ...
    log.info("=" * 68)
    log.info("  PORTFOLIO-OPTIMIERUNG V4 | MARKOWITZ (LW) vs. RANDOM FOREST")
    log.info("  Korrekturen: Look-Ahead-Bias | Max-Weight 20% | Transaktionskosten")
    log.info("  Neu v4: RF Turnover-Constraint (max. 30%/Monat) | Frontier-Evolution")
    ...
    frontier_snapshots = results["frontier_snapshots"]   # v3b
