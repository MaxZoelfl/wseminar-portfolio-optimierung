...
Version 2.0 — erweiterte Methodik
=============================================================================
...
class MarkowitzLedoitWolf:
    ...
    def max_sharpe(self, mu: np.ndarray, cov: np.ndarray) -> np.ndarray:
        ...
        MAX_WEIGHT = 0.25  # Maximale Prozent je Asset (konfigurierbar)
...
def run_backtest(asset_prices: pd.DataFrame,
    ...
    for i, month_end in enumerate(backtest_months[:-1]):
        ...
        # ---- Random Forest ------------------------------------------------
        # Features: alle Trainingsmonats-Zeilen fuer alle Assets zusammenfuehren
        feat_rows   = []
        target_rows = []
        ...
        results_mvo.append((hold_period * w_mvo).sum(axis=1))
        results_rf.append( (hold_period * w_rf ).sum(axis=1))
        results_ew.append( (hold_period * w_ew ).sum(axis=1))
...
def save_all_csv(returns_df: pd.DataFrame,
    ...
    log.info("  -> 5 CSV-Dateien gespeichert.")
...
def main():
    ...
    log.info("=" * 68)
    log.info("  PORTFOLIO-OPTIMIERUNG V2 | MARKOWITZ (LW) vs. RANDOM FOREST")
